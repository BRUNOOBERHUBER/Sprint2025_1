...
import { listarContraturno, criarContraturno, getContraturno, updateContraturno, Contraturno } from "../../api/contraturno";
import { listarAlunos, Aluno } from "../../api/alunos";
import { listarColaboradores, Colaborador } from "../../api/colaboradores";
...
export default function ContraturnoFormPage() {
  ...
  const { data: alunos = [] } = useQuery<Aluno[]>({ queryKey: ["alunos"], queryFn: listarAlunos });
  const { data: colaboradores = [] } = useQuery<Colaborador[]>({ queryKey: ["colaboradores"], queryFn: listarColaboradores });

  // Se editando, carrega dados existentes e preenche o formulário
  const {
    data: projetoData,
    isLoading: isLoadingProjeto,
  } = useQuery<Contraturno>(
    ["contraturno", id],
    () => getContraturno(id!),
    { enabled: !!id }
  );
  ...
  useEffect(() => {
    if (projetoData) {
      reset({
        ...
        professor: projetoData.professor || "",
  ...
  const mutation = useMutation({
    mutationFn: (data: FormValues) => {
      return id ? updateContraturno(id, data) : criarContraturno(data);
    },
    onSuccess: () => {
