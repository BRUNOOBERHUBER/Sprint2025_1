...
import { listarContraturno, deleteContraturno, updateContraturno, Contraturno } from "../../api/contraturno";
...
export default function ContraturnoProjetos() {
  ...
  const [editProjeto, setEditProjeto] = useState<Contraturno | null>(null);
  const [formEdit, setFormEdit] = useState<Partial<Contraturno>>({});
  const { data: projetos = [], isLoading } = useQuery<Contraturno[]>({ queryKey: ["contraturno"], queryFn: listarContraturno });
  const { data: allAlunos = [] } = useQuery<Aluno[]>({ queryKey: ["alunos"], queryFn: listarAlunos });
  const [searchTerm, setSearchTerm] = useState("");
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [projetoToDelete, setProjetoToDelete] = useState<Contraturno | null>(null);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: Partial<Contraturno> }) => updateContraturno(id, updates),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contraturno"] });
      setEditProjeto(null);
    },
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded p-6 w-full max-w-md">
            ...
            <div className="flex justify-end space-x-2">
              <Button variant="outline" onClick={() => { setProjetoToDelete(null); setDeleteConfirmationText(""); }}>Cancelar</Button>
              <Button
                variant="primary"
      ...
      {editProjeto && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded p-6 w-full max-w-md">
            ...
            <form onSubmit={(e) => { e.preventDefault(); updateMutation.mutate({ id: editProjeto._id!, updates: formEdit }); }} className="space-y-4">
              <div>
              ...
              <div>
                ...
                <textarea value={formEdit.descricao} onChange={(e) => setFormEdit({ ...formEdit, descricao: e.target.value })} className="border rounded w-full p-2" />
              </div>
              <div>
                <label className="block font-medium">Professor</label>
                <Input value={formEdit.professor} onChange={(e) => setFormEdit({ ...formEdit, professor: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                ...
                <Input type="number" value={formEdit.vagas} onChange={(e) => setFormEdit({ ...formEdit, vagas: Number(e.target.value) })} />
              </div>
              <div>
                <label className="block font-medium">Categoria</label>
                <Input value={formEdit.categoria} onChange={(e) => setFormEdit({ ...formEdit, categoria: e.target.value })} />
              </div>
              <div>
                <label className="block font-medium">Status</label>
                <select value={formEdit.status} onChange={(e) => setFormEdit({ ...formEdit, status: e.target.value })} className="border rounded w-full p-2">
                  <option value="Ativo">Ativo</option>
                  <option value="Inativo">Inativo</option>
                </select>
              </div>
              <div className="flex justify-end space-x-2">
                <Button variant="outline" onClick={() => setEditProjeto(null)}>Cancelar</Button>
