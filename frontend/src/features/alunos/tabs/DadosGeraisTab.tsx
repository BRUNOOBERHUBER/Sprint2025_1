...
import { getAluno, updateAluno, AlunoUpdate, ContatoResponsavel, Aluno } from "../../../api/alunos";
...
export default function DadosGeraisTab() {
  ...
  const { mutateAsync, isPending: saving } = useMutation({
    mutationFn: (payload: AlunoUpdate) => updateAluno(id!, payload),
    onError: (error: any) => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (values.enderecoCompleto) {
      payload.dadosPessoais = { enderecoCompleto: values.enderecoCompleto } as any;
    }
    ...
    if (contatosRespLimpos.length) {
      payload.contatosResponsaveis = contatosRespLimpos as ContatoResponsavel[];
    }
  ...
  if (isError) return <p className="text-red-500">Erro ao buscar aluno: {(error as Error).message}</p>;
  if (!aluno) return <p className="text-red-500">Aluno não encontrado.</p>;

  return (
    <div>
      <StudentProfile student={aluno as any} onEdit={() => setEditMode(true)} />
          ...
          <div>
            ...
            {respFields.map((field, index) => (
              <div key={field.id} className="border p-3 mb-2 rounded relative">
                ...
                <input {...register(`contatosResponsaveis.${index}.nome` as const)} placeholder="Nome" className="border rounded w-full p-1 mb-1" />
                <input {...register(`contatosResponsaveis.${index}.fone` as const)} placeholder="Telefone" className="border rounded w-full p-1 mb-1" />
                <input {...register(`contatosResponsaveis.${index}.email` as const)} placeholder="E-mail" className="border rounded w-full p-1" />
              </div>
            ))}
